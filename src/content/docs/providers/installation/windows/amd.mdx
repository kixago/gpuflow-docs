...
description: GPUFlow provider installation for Windows with AMD GPU — step-by-step setup using WSL2 and ROCm to list your GPU and earn passive income.
sidebar:
    label: 'Windows + AMD'
    order: 7
---
...
```bash
docker pull docker.io/gpuflow/universal:latest
...
docker run -d \
  ...
  --network=host \
  -v /opt/gpuflow:/data \
  -e GPUFLOW_MACHINE_ID="YOUR_MACHINE_ID" \
  -e RELAY_SERVER_URL="wss://gpuflow.app/ws/container" \
  -e MAIN_SITE_URL="https://gpuflow.app" \
  -e GPU_VENDOR="amd" \
  docker.io/gpuflow/universal:latest
```
> Your Machine ID is shown in the GPUFlow dashboard under **Dashboard → Containers**. Add your machine ID there first, then copy it into the command above.
