...
description: Fix common GPUFlow provider issues on Linux — container runtime errors, GPU detection problems, WireGuard connectivity, and more.
sidebar:
...
**Missing API key:**
```bash
# Verify your GPUFLOW_MACHINE_ID is set correctly
docker run -d \
...
docker inspect gpuflow-provider | grep -A 5 '"Env"'
podman inspect gpuflow-provider | grep -A 5 '"Env"'
```
