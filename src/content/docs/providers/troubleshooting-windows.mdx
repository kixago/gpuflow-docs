...
description: Fix common GPUFlow provider issues on Windows — WSL2 setup errors, GPU passthrough problems, and connectivity troubleshooting.
sidebar:
...
**Machine ID issues:**
```bash
# Verify Machine ID and relay URL are set correctly
docker inspect gpuflow-provider | grep -A 10 '"Env"'

# Update with correct values
docker stop gpuflow-provider
docker rm gpuflow-provider
# Re-run docker command with correct Machine ID
```
...
docker run -d \
  ...
  --cpus="2.0" \
  -v /opt/gpuflow:/data \
  -e GPUFLOW_MACHINE_ID="your-machine-id-from-dashboard" \
  -e RELAY_SERVER_URL="wss://gpuflow.app/ws/container" \
  -e MAIN_SITE_URL="https://gpuflow.app" \
...
<Aside type="tip">
Most Windows issues stem from WSL2 configuration, driver problems, or an incorrect Machine ID. Verify your GPUFLOW_MACHINE_ID matches exactly what's shown in your dashboard under Containers.
</Aside>
